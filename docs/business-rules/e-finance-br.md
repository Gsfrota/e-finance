...
- **Condição:** Aba "Perguntar" do `AdminAssistant` (`AppView.ASSISTANT`) — tela de **conversa** com o assistente (`components/ui/animated-ai-chat.tsx`, componente de apresentação controlado); acesso exclusivo ao perfil `admin`. O nome exibido do assistente vem de `bot_tenant_config.ai_persona_name` por tenant (Salomão, Elisa, ...), nunca hardcoded
- **Resultado:** Uma mensagem de chat em texto (`services/assistantAnswer.ts`), com as duas janelas, cada uma com valor em BRL + quantidade de contratos, e a nota de que renovações contam. O texto usa `*asterisco*` como negrito — mesmo formato do bot no WhatsApp; a UI renderiza como negrito real. Escopo = `tenant_id` do admin + `company_id` da empresa ativa quando houver uma selecionada (escopo agregado `'all'` soma o tenant inteiro), idêntico ao resto do app. **Renovações contam como empréstimo** — contrato com `parent_investment_id` preenchido entra na soma, porque renovar é emprestar aquele dinheiro novamente. Motor e janelas são funções puras testadas em `tests/unit/assistantEngine.test.ts`
...
- **Tabelas:** `investments` (leitura: `amount_invested`, `created_at`, `tenant_id`, `company_id`), `bot_tenant_config` (leitura: `ai_persona_name`)
- **Status:** ativa
- **Stories:** implementa BR-BOT-009 em 12/09/2026; vira tela de conversa em 13/09/2026
- **Evidência:** caso de borda validado na UI real contra oráculo SQL independente em 13/09/2026 — 8 contratos / R$ 22.934,56, com limite de 1 segundo (domingo 23:59:59 fora), `company_id` NULL fora do escopo, `completed`/`defaulted` dentro, renovações dentro

### BR-BOT-010: Assistente determinístico — catálogo de perguntas e período na frase
- **Descrição:** Além do volume emprestado (BR-BOT-009), o Assistente responde **sem LLM** a quatro perguntas, cada uma casada por regex e traduzida em UMA consulta exata: (1) **`late_debtors`** — "quem está atrasado" → parcelas vencidas; (2) **`receivables`** — "quanto tenho pra receber" → parcelas a vencer na janela; (3) **`received`** — "quanto recebi" → parcelas efetivamente pagas na janela; (4) **`debtor_balance`** — "quanto o Fulano me deve" → saldo em aberto daquele cliente; (5) **`received_from_debtor`** — "quanto o Fulano já me pagou" → pagamentos daquele cliente. A frase também carrega o **período**: `hoje`, `ontem`, `essa semana`, `últimos N dias`, `esse mês`, `próximos N dias`. Período ausente usa o padrão da intent: `lent_volume` → semana corrente + últimos 7 dias; `received` → hoje; `receivables` → próximos 7 dias; `late_debtors` e `debtor_balance` → sem período (é uma foto do agora)
- **Condição:** Aba "Perguntar" do `AdminAssistant` (`AppView.ASSISTANT`); exclusivo do perfil `admin`; escopo `tenant_id` + `company_id` da empresa ativa, como o resto do app
- **Resultado:** Resposta em **tom de conversa** (não bloco de relatório), com o número e a janela explícitos, mais uma **sugestão de próxima pergunta** (`followUp`) que o chat oferece como botão clicável. Definição de atraso segue BR-REL-005: `due_date < hoje AND status IN ('pending','late','partial')`, excluindo contratos `completed`/`renewed`. `received` soma por `paid_at`; `receivables` e `late_debtors` por `due_date`. Parcelas fantasma (BR-REL-002: `amount_total = 0 AND amount_paid = 0 AND status = 'paid'`) são sempre excluídas. `debtor_balance` casa o nome por correspondência parcial, sem acento e sem caixa
- **Exceções:** Pergunta que não casa nenhuma intent continua respondendo "não entendi" + lista do que é suportado, **sem consultar o banco**. Nome de cliente ambíguo (mais de um bate) devolve a lista de nomes e pede para escolher — **nunca escolhe sozinho**. Nome não encontrado diz isso, em vez de responder R$ 0,00 (zero por ausência de cadastro é diferente de zero por não dever nada). Período fora do suportado (ex.: "no ano passado") recusa em vez de silenciosamente usar outro. A variação de tom é **fixa por intent**, nunca aleatória: mesma pergunta ⇒ mesmo texto
- **O vocabulário vem do tráfego real, não do que a gente imagina:** o léxico foi ampliado contra as 447 conversas reais de `bot_turn_traces` (donos de operação no WhatsApp). Daí saem quatro exigências que a escrita de celular impõe — typo por letra repetida ("devenndo"), abreviação ("qto", "hj", "vc", "q", "p"), verbo cortado ("recebe" por "receber") e **ordem invertida** ("Damião deve quanto"). O nome do cliente casa nos **dois sentidos**: o dono escreve o apelido sobre um cadastro curto ("João da Silva bom bom") e o primeiro nome sobre um cadastro longo. Nome com mais de 6 palavras ou 40 caracteres não é nome — é uma frase que o padrão pegou errado, e a intent segue sem ele
- **Nome citado junto de "pagou" é pergunta sobre aquele cliente** (`received_from_debtor`), nunca o caixa do período: "quanto o João já me pagou" soma os pagamentos DELE — a vida inteira quando não há período na frase, porque "já me pagou" não tem janela. Sem nome, "quanto foi pago hoje" continua sendo o total do dia. Antes desta regra a frase caía em `received` e devolvia o total de todos os clientes como se fosse a resposta sobre a pessoa citada — errar assim é pior que recusar, porque o número parece certo
- **Escopo de empresa vem do CONTRATO, não do cadastro:** a busca de `debtor_balance` filtra `profiles` só por `tenant_id` + `role='debtor'` — **nunca** por `company_id`. Cliente antigo tem `profiles.company_id` nulo enquanto o contrato dele carrega a empresa; filtrando o cadastro, o Assistente listava o cliente em "quem está atrasado" (escopo vindo da parcela) e negava a existência dele em "quanto o Fulano me deve". O recorte por empresa é aplicado nas parcelas, que é onde ele existe de verdade
- **"A receber essa semana" olha pra frente:** recebível usa a janela `week_remainder` (hoje 00:00 BRT até segunda 00:00), não a `current_week` retrospectiva do volume emprestado — parcela que venceu na segunda passada é atraso, não previsão de entrada. No domingo, a janela é só o próprio dia
- **Volume emprestado responde o par de janelas:** "essa semana" e "últimos 7 dias" dão números diferentes, então `lent_volume` devolve os dois lado a lado quando a pergunta cita qualquer uma das duas (ou nenhuma). Só período de outra natureza ("esse mês", "ontem", "últimos 15 dias") responde sozinho
- **Tabelas:** `investments`, `loan_installments` (leitura), `profiles` (leitura: `full_name` para `debtor_balance`)
- **Status:** ativa
...
- **Conversa sobrevive à navegação:** o histórico do chat vive fora do componente (módulo, em memória), porque abrir um contrato pela lista **sai da tela do Assistente** e desmontaria a conversa. Ao voltar, a conversa reaparece já rolada na última resposta. É memória de sessão: recarregar a página, sair da conta ou trocar de tenant zera — nada de cliente ou valor é gravado no navegador
