  ...
  if (loading) {
      return (
        <div className="flex flex-col items-center justify-center h-full space-y-4 animate-pulse">
            ...
            <p className="type-label text-[color:var(--text-muted)]">Carregando seus contratos...</p>
        </div>
      );
  }
      ...
      <div
        ...
        className={`p-6 md:p-8 rounded-[2.5rem] border shadow-2xl flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden transition-all duration-500 ${
          ...
          <div className="relative z-10 text-center md:text-left">
              <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full type-label mb-3 ${
              ...
              <div data-testid="next-payment-card" className="relative z-10 bg-slate-950/60 backdrop-blur-md border border-white/10 p-6 rounded-3xl w-full md:w-80 shadow-lg transform hover:scale-[1.02] transition-transform">
                  <div className="flex justify-between items-start mb-4">
                      ...
                      {metrics.nextPayment.is_late && (
                          <span className="bg-red-500 text-white type-micro px-2 py-1 rounded animate-pulse">Atrasado</span>
                      )}
                  </div>
                  <div className="mb-6">
                  ...
                  <button
                    ...
                    className={`w-full min-h-[44px] py-3 rounded-xl type-label flex items-center justify-center gap-2 transition-all shadow-lg active:scale-95 ${
                        metrics.nextPayment.is_late 
                  ...
                  <div data-testid="contract-item" key={contract.id} className={`bg-slate-800 rounded-[2rem] border transition-all duration-300 overflow-hidden ${
                      ...
                      >
                          <div className="flex items-start gap-4">
                              ...
                              <div>
                                  <h4 className="type-subheading text-[color:var(--text-primary)]">{contract.asset_name}</h4>
                                  <div className="flex items-center gap-2 mt-1">
                                    <span className={`type-label px-2 py-0.5 rounded ${
                          ...
                          <div className="flex items-center gap-6 justify-between md:justify-end w-full md:w-auto">
                              <div className="text-right">
                                  ...
                                  <p className="type-metric-lg text-[color:var(--text-primary)]">{formatCurrency(contract.balance)}</p>
                              </div>
                              
                              {/* Desktop Progress Bar */}
                              <div className="hidden md:block w-32">
                                   <div className="flex justify-between type-micro text-[color:var(--text-muted)] mb-1">
                                       <span>Progresso</span>
                                          ...
                                          {contract.installments.map(inst => (
                                              <tr data-testid="installment-row" key={inst.id} className="hover:bg-slate-800/50 transition-colors">
                                                  ...
                                                  <td className="py-4 text-right">
                                                      <span className="font-semibold text-[color:var(--text-primary)]">{formatCurrency(inst.amount_total)}</span>
                                                  </td>
                                                  <td className="py-4 text-center hidden sm:table-cell">
                                                      {inst.status === 'paid' ? (
                                                          <span data-testid="installment-status" className="text-green-400 type-label flex items-center justify-center gap-1"><CheckCircle2 size={12}/> Pago</span>
                                                      ) : inst.is_late ? (
                                                          <span data-testid="installment-status" className="text-red-400 type-label flex items-center justify-center gap-1"><AlertTriangle size={12}/> Atrasado</span>
                                                      ) : (
                                                          <span data-testid="installment-status" className="type-label text-[color:var(--text-muted)]">A Vencer</span>
                                                      )}
                                                  </td>
                                                  <td className="py-4 pr-4 text-right">
                                                      {inst.status !== 'paid' && (
                                                          <button
                                                            ...
                                                            className={`min-h-[44px] px-4 py-2 rounded-lg type-label flex items-center gap-2 float-right transition-all ${
                                                                inst.is_late
