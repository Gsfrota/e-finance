...
export function parseConfirmationReply(text: string): 'confirm' | 'cancel' | null {
  ...
  if (/^(sim|confirmo|ok|pode|isso|s|segue|pode seguir|tá|ta|certo|beleza|blz|perfeito|isso mesmo|pode ser|pode confirmar|bora|combinado|yes)$/.test(normalized)) return 'confirm';
