...
export async function getOpenInstallmentsByDebtorName(
  tenantId: string,
  debtorNameQuery: string,
  limit = 6,
): Promise<{ installments: ContractOpenInstallment[]; matchedDebtors: number }> {
  const { data: debtors } = await db()
    .from('profiles')
    .select('id, full_name')
    .eq('tenant_id', tenantId)
    .eq('role', 'debtor')
    .ilike('full_name', `%${debtorNameQuery.trim()}%`)
    .limit(5);

  if (!debtors?.length) return { installments: [], matchedDebtors: 0 };

  const debtorIds = debtors.map(d => String(d.id));

  const { data: investments } = await db()
    .from('investments')
    .select('id')
    .eq('tenant_id', tenantId)
    .in('payer_id', debtorIds)
    .eq('status', 'active');

  if (!investments?.length) return { installments: [], matchedDebtors: debtors.length };

  const { data: rows } = await db()
    .from('loan_installments')
    .select(`
      id, number, investment_id, amount_total, amount_paid, due_date, status,
      investments!inner(id, tenant_id, payer_id, debtor:profiles!investments_payer_id_fkey(full_name))
    `)
    .eq('investments.tenant_id', tenantId)
  ...
  const installments = (rows ?? []).map(row =>
    mapContractOpenInstallmentRow(row, Number((row as any).investment_id)),
  );

  return { installments, matchedDebtors: debtors.length };
}
